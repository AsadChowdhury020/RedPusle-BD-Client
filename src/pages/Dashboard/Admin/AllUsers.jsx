...
const AllUsers = () => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (_data, { action }) => {
      const messages = {
        ...
        volunteer: "User is now a Volunteer.",
        admin: "User is now an Admin.",
        donor: "User is now a Donor.",
      };

      Swal.fire("Success", messages[action], "success");
      queryClient.invalidateQueries({ queryKey: ["all-users"] });
    },
    onError: () => {
      Swal.fire("Error", "Failed to update user.", "error");
  ...
  if (isLoading) return <LoadingSpinner />;
  if (isError)
    return <p className="text-error">Failed to load users.</p>;

  return (
    <div className="p-6 bg-base-100 border border-base-300 rounded-xl mt-6">
      ...
      <div className="flex justify-end mt-4 space-x-2 items-center">
        ...
        <span className="px-2 py-1 text-base-content/70">
          Page {currentPage} of {totalPages}
        </span>
